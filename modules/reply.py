...
#如果要修改套件內容，直接在這份檔案上面編輯
#修正完成要回app.py啟動，不是啟動這份檔案
# 常見問答表
faq = {
    "簡介":TextMessage(text="我是戴律師的小助手，輸入menu開始使用"),
    "貼圖": StickerMessage(
        package_id="789",
        sticker_id="10858"
    ),
    ...
    "捷運": TextMessage(
        text="搭乘捷運至臺大醫院站步行5分鐘即可抵達。"
    ),
    "公車": TextMessage(
        text="搭乘公車至臺大醫院站步行5分鐘即可抵達。"
    ),
...
menu = TemplateMessage(
    alt_text="請更新LINE版本已支援選單格式",
    #如果使用者使用的版本過舊時會出現的提示
    template=CarouselTemplate(
        #一個carouselcolumn就是一張選單卡片
        columns=[
            CarouselColumn(
                # 卡片一圖片網址
                ...
                text="點選下方按鈕開始互動",
                actions=[
                    #一張卡片只能放三個按鈕action，不能放4個，多的要放第二張卡片
                    MessageAction(
                        label="查詢匯率",
                        text="查詢匯率"
                        #label是按鈕標題，text是使用者點選後的回應
                    ),
                    MessageAction(
                        label="營業時間",
                        text="營業時間"
